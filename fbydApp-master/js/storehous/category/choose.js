...
function chooseFn(menu10,menu10Html){
	//筛选
	var menu10=menu10;
...
window.addEventListener("closeMenuMask",function(){
	alert("关闭遮罩层的事件")
	_closeMenu();
...
//	alert("关闭遮罩层");
	...
	if(showMenu) {
		...
		document.getElementsByClassName("kOrderBlock")[0].style.backgroundColor="#fff";
...
function sxTextFn(index) {  
	var main = plus.webview.currentWebview().opener(); //获取父页面A对象
	mui.fire(main,"changeBackground");//改变入口文件背景颜色rgba(0, 0, 0, 0.3)
	
	zhpxFlag = false;
	openMenu();
	document.getElementsByClassName("kOrderBlock")[0].style.backgroundColor="rgba(0, 0, 0, 0.3)";
